...
4. Self-hosted CI является validation-only:
   - не устанавливает и не обновляет Node.js, Rust, rustfmt, Clippy, MSVC, NSIS или другие системные инструменты;
   - не использует setup actions, `rustup install/update/component add`, `winget`, Chocolatey, Scoop, `msiexec`, `RunAs` или запуск setup-файлов;
   - при отсутствии инструмента падает с инструкцией по ручной подготовке runner.
...
6. npm-зависимости разрешено восстанавливать только внутрь checkout через `npm ci --ignore-scripts`, с process-scoped registry/cache.
7. Обычный `Release Quality` не собирает NSIS installer. Installer создается только trusted release workflow при наличии заранее установленного `makensis.exe` и никогда не запускается.
8. После загрузки нужных artifacts/diagnostics удалять generated `node_modules`, frontend `dist`, Rust `target` и staged release directories.
9. Единственное исключение из self-hosted assignment — изолированный `publish-release` job:
...
10. Любое изменение workflow должно сопровождаться обновлением `scripts/test-workflow-contracts.mjs`.
