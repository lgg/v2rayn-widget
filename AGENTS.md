...
4. На persistent runner нельзя менять npm registry/cache глобально. Использовать process-scoped environment variables и runner temp.
5. После загрузки нужных artifacts/diagnostics удалять generated `node_modules`, frontend `dist`, Rust `target` и staged release directories.
6. Единственное исключение из self-hosted assignment — изолированный `publish-release` job:
...
7. Любое изменение workflow должно сопровождаться обновлением `scripts/test-workflow-contracts.mjs`.
